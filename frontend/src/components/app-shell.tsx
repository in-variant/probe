...
function LoginScreen({ onLogin }: { onLogin: () => void }) {
  return (
    <div className="paper-grain relative flex h-dvh items-center justify-center overflow-hidden bg-background">
      <div className="pointer-events-none absolute -left-20 top-8 h-64 w-64 rounded-full bg-violet-200/40 blur-3xl" />
      <div className="pointer-events-none absolute -right-24 bottom-0 h-72 w-72 rounded-full bg-sky-200/45 blur-3xl" />
      <div className="w-full max-w-sm px-6">
        <div className="rounded-2xl border border-zinc-200/80 bg-white/90 p-6 shadow-sm backdrop-blur">
...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  const showComplianceNav =
    user.role === "INVARIANT" || user.role === "ADMIN" || user.role === "CLIENT";
